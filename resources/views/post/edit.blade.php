    ...
    <form method="POST" action="/blog/{{ $post->id }}">
        ...
        <div class="space-y-12">
            <div class="border-b border-gray/10 pb-12">
                ...
                <p class="mt-1 text-sm/6 text-gray-400">Use this form to update post data for the blog.</p>
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            <input id="author" value="{{ old('author', $post->author) }}" type="text"
                                name="author" autocomplete="family-name"
                    ...
                    <div class="col-span-full">
                        ...
                        <div class="mt-2">
                            <textarea id="body" name="body" rows="3"
                                class="{{ $errors->has('body') ? 'outline-red-500 focus:outline-red-600' : 'outline-gray-500' }}block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 placeholder:text-white-500 focus:outline-indigo-500 focus:outline-2 focus:-outline-offset-2 sm:text-sm">
                              {{ old('body', $post->body) }}
                            </textarea>
                        </div>
                        <p class="mt-3 text-sm/6 text-gray-500">Write a few sentences about the article.</p>
                        @error('body')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                            ...
                            <div class="flex h-6 shrink-0 items-center">
                                <div class="group grid size-4 grid-cols-1">
                                    <input id="published" type="checkbox" name="published" value="1"
                                        ...
                                        class="col-start-1 row-start-1 appearance-none rounded-sm border border-gray/10 bg-white/5 checked:border-indigo-500 checked:bg-indigo-500 indeterminate:border-indigo-500 indeterminate:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 disabled:border-gray/5 disabled:bg-white/10 disabled:checked:bg-white/10 forced-colors:appearance-auto" />
